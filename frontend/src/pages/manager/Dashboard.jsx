        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {cycle ? `${cycle.year} · ${cycle.phase}` : 'Active cycle'} · {team.length} direct reports
            </p>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-xl border border-yellow-200 p-4 text-center">
            <p className="text-2xl font-semibold text-yellow-600">{totalPending}</p>
            <p className="text-xs text-gray-500 mt-1">Pending approval</p>
          </div>
          <div className="bg-white rounded-xl border border-green-200 p-4 text-center">
            <p className="text-2xl font-semibold text-green-600">{totalApproved}</p>
        ...
        {totalPending > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-5 py-3">
            <p className="text-yellow-800 text-sm font-medium">
              {totalPending} goal{totalPending !== 1 ? 's' : ''} awaiting your review
            </p>
          </div>
        )}
        ...
        {team.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="font-medium text-gray-900 mb-2">No direct reports found</h3>
          ...
          <div className="space-y-4">
            {team.map(({ employee, goals, totalWeightage, submittedCount, approvedCount }) => (
              <EmployeeGoalCard
                key={employee.id}
                employee={employee}
                goals={goals}
