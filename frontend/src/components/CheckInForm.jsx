...
export default function CheckInForm({ goal, quarter, existingCheckIn, onSaved }) {
  ...
  const [completionDate,  setCompletionDate] = useState(
    existingCheckIn?.completionDate
      ? new Date(existingCheckIn.completionDate).toISOString().split('T')[0]
      : ''
  );
  const [progressStatus,  setProgressStatus] = useState(
    existingCheckIn?.progressStatus || 'NOT_STARTED'
  );
    ...
    <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm font-semibold text-gray-900 mt-0.5">{goal.title}</p>
      ...
      {liveScore !== null && (
        <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-gray-200">
          ...
          <span className="text-xs text-gray-400 ml-auto">
            {goal.uomType === 'NUMERIC_MIN' && `${actual} ÷ ${goal.target} × 100`}
            {goal.uomType === 'NUMERIC_MAX' && `${goal.target} ÷ ${actual} × 100`}
            {goal.uomType === 'ZERO'        && (parseFloat(actual) === 0 ? 'Zero achieved ✓' : 'Non-zero value')}
            {goal.uomType === 'TIMELINE'    && (liveScore === 100 ? 'On or before deadline ✓' : 'Past deadline')}
          </span>
        </div>
      )}
      ...
      {existingCheckIn?.managerComment && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
          <p className="text-xs font-medium text-amber-700 mb-1">Manager comment</p>
          <p className="text-xs text-amber-800">{existingCheckIn.managerComment}</p>
        </div>
      )}
